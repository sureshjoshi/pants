...
async def _determine_import_from_candidates(
    ...
    # return Addresses([Address(spec_path="src", target_name=None, generated_name=None, relative_file_path=None)])
    # return Addresses([Address(spec_path="src", target_name="", generated_name=None, relative_file_path=None)])
    
    # Compiles and runs, does nothing of substance
    # return Addresses([Address(spec_path="src", target_name="wtf", generated_name=None, relative_file_path=None)])

    # Exception: Only a file Address (`self.is_file_target`) has a filename: //:package_json#pants-slow-js-deps-demo
    # return Addresses([Address(spec_path="src", target_name="wtf", generated_name=None, relative_file_path="file0002.js")])
    
    # ResolveError: The address `src:wtf#file0002.js` from <infallible> was not generated by the target `src:wtf`. Did you mean one of these addresses?
    # * src:wtf
    # * src/file0001.js:wtf
    # * src/file0002.js:wtf
    # return Addresses([Address(spec_path="src", target_name="wtf", generated_name="file0002.js", relative_file_path=None)])

    # Runs, but can't be doing anything of value
    # return Addresses([Address(spec_path="src", target_name="wtf", generated_name=None, relative_file_path=None)])

    # ResolveError: Directory 'src/file0002.js' does not contain any BUILD files.
    # return Addresses([Address(spec_path="src/file0002.js", target_name=None, generated_name=None, relative_file_path=None)])

    #ResolveError: Directory 'src/file0002.js' does not contain any BUILD files.
    # return Addresses([Address(spec_path="src/file0002.js", target_name="wtf", generated_name=None, relative_file_path=None)])

    # The target name ':file0002.js' is not defined in the directory src. Did you mean one of these target names?
    #   * :wtf
    # return Addresses([Address(spec_path="src", target_name="wtf", relative_file_path="file1000")])
    
    # ===> This might work, if the files are named as `javascript_source(...)`
    # return Addresses([Address("src", target_name="file0002")])

    # BYPASS THE WORLD
    file_import = next(iter(candidates.file_imports))
    return Addresses([Address("src", target_name=file_import.lstrip("src/").rstrip(".js"))])
    ...
    # logger.warning(f"Candidates: {candidates} --> addresses: {addresses}")
